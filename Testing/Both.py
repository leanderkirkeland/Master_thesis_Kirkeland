...
for i in range(20):
    for j in range(100):
        data = xr.open_dataset(fr"Both/Both_h_{i}_{j}.nc")
...
for i in range(20):
    for j in range(100):
        data = xr.open_dataset(fr"Both/Both_l_{i}_{j}.nc")
...
with open("Pred_both_h.npy", "wb") as f:
    np.save(f, pred1)

with open("Pred_both_l.npy", "wb") as f:
    np.save(f, pred2)
...
for j in range(20):
    vel_y1 = []
    for i in range(100):
        data = xr.open_dataset(fr"Both/Both_h_{j}_{i}.nc")
        vel_y1.append(data["blob_labels"])
    vel_y.append(vel_y1)

with open("Pred_both_h.npy", "rb") as f:
    x = np.load(f)
...
with open("Pred_both_h_measures.npy", "wb") as f:
    np.save(f, measures)
...
plt.title("Accuracy with varying x-velocities and sizes")
...
plt.title("IoU with varying x-velocities and sizes")
plt.xlabel("Number of blobs")
plt.ylabel("IoU")
plt.xticks(np.arange(1,21))
plt.legend()
