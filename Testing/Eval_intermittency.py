...
for j in range(20):
    ...
    vel_y1 = []
    for i in range(100):
        data = xr.open_dataset(fr"Intermittency/Intermittency_l_{j}_{i}.nc")
        vel_y1.append(data["blob_labels"])
    vel_y.append(vel_y1)

with open("Pred_int_l.npy", "rb") as f:
    x = np.load(f)
...
np.where(vel_y[0,0] != 0)

plt.imshow(vel_y[0,0,60])
plt.show()
plt.imshow(x1[0,0,60])
plt.show()
...
with open("Pred_int_l_measures.npy", "wb") as f:
    np.save(f, measures)

with open("Pred_int_h_measures.npy", "rb") as f:
    meas1 = np.load(f, allow_pickle=True)

with open("Pred_int_l_measures.npy", "rb") as f:
...
plt.title("Accuracy with varying sizes")
...
plt.title("IoU with varying sizes")
plt.xlabel("Number of blobs")
plt.ylabel("IoU")
plt.xticks(np.arange(1,21))
plt.legend()
